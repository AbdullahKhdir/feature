...
export = class Shop extends BaseController {
	 ...
	 */
	products = () =>
		this.route("get", "/products/", { userSession }, async (req: Request, res: Response, next: NextFunction) => {
			if (!req.isGet()) {
			...
			if (typeof user_products === "object") {
				user_products
					.then((rows: any) => {
						...
						return this.render(res, "shop/product-list", {
							products: records,
							nav_title: "Products",
							path: "/products/",
							root: "shop",
	 ...
	 */
	index = () =>
		this.route("get", "/", {}, async (req: Request, res: Response, next: NextFunction) => {
			if (!req.isGet()) {
				return this.siteNotFound(res);
			}
			...
			// const descCollection = await testModel.describeCollection();
			// const validationCollection = await testModel.createValidatedCollection();
			// console.log(descCollection);
			// console.log(validationCollection);
			// await testModel.createUser({
			// 	user: "testUser", // MongoDB user
			// 	pwd: "123456789", // Password for johnDoe
			// 	roles: [{ role: "read" }, { role: "userAdmin" }] // Read-only and user can edit roles for other users
			// });
			// const connection = await Singleton.pool.getConnection();
			// await connection.beginTransaction();
			// const result = await cartItem.create({ cart_id: 1, product_id: 1, quantity: 3 }, connection);
			...
			if (typeof user_products === "object") {
				user_products
					.then((rows: any) => {
						...
						return this.render(res, "shop/index", {
							products: records || [],
							nav_title: "shop",
							path: "/",
							root: "shop",
										...
										if (cart_products["getProducts"].length > 0) {
											...
											if (!this._.isEmpty(cart_products)) {
												...
												this.product
														...
														return {
															session: async () => {
																const stripe = require("stripe")(
																	"sk_test_51LwhSpHh4oQqwjTUy0a2jgwphD8CTgb8czMsySR79Jbqfwjm2M819AO3ydmvQfybePlZixdlFl0fE0ur40UE3xMw00Uinv9DFm"
																);
													...
													.then((data: any) => {
														return this.render(res, "shop/cart", {
															nav_title: "My Cart",
															path: "/cart/",
						...
						.then((rows: any) => {
							if (!this._.isEmpty(rows)) {
								...
								this.cart_items_object
									.filter({ cart_id: cart_id, product_id: product_id })
									// @ts-ignore
	...
	getCheckout = () =>
		this.route(
			...
			async (req: Request, res: Response, next: NextFunction) => {
				...
				res.updatedContentAlways();
				return this.render(res, "shop/checkout", {
					nav_title: "Checkout",
					path: "/checkout/"
				});
			}
									...
									if (!this._.isEmpty(ordered_products.getProducts)) {
										...
										this.product
											...
											.then((_products: any) => {
												...
												return this.render(res, "shop/orders", {
													nav_title: "My Orders",
													path: "/orders/",
										...
										products.forEach((product: any) => {
											if (product) {
												...
												this.order_items_object
													.filter({ order_id: order_id, product_id: product.product_id })
					...
					this.product
						...
						.then((rows: any) => {
							if (rows) {
								...
								return this.render(res, "shop/product-detail", {
													...
													if (_user) {
														...
														if (_result["order_products_items"] && user.id) {
															condition =
																...
																][0].user_id.toString() === user.id.toString();
														}
														if (condition) {
															const invoice = `invoice_${order_id}.pdf`;
															const path = Singleton.getPath().join(
																__dirname,
																"..",
																"..",
																"public",
