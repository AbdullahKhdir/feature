...
export = class Admin extends BaseController {
        ...
        */
		...
		this.upload_middleware = Singleton.configUploader({
			...
			file_filter: (req: Request, file: Express.Multer.File, callback: multer.FileFilterCallback) => {
				if (
					file.mimetype === Singleton.getConstants().RESPONSE.TYPES.PNG ||
					file.mimetype === Singleton.getConstants().RESPONSE.TYPES.JPEG ||
					file.mimetype === Singleton.getConstants().RESPONSE.TYPES.JPG
				) {
					callback(null, true);
					...
					//! FILE WILL NOT BE SAVED
					req.sendFormPostedData();
					return this.onErrorValidation(
						// @ts-ignore
						req.res,
						"Only images with the (PNG, JPEG or JPG) extensions are allowed"
					);
				}
			},
			// @ts-ignore
		...
		this.route(
			...
			async (req: Request, res: Response, next: NextFunction) => {
				if (req.isGet()) {
					...
					return this.render(res, "admin/add-product", {
	...
	editProduct = () =>
		this.route(
			...
			async (req: Request, res: Response, next: NextFunction) => {
				...
				const product_id = +req.getDynamicParam("product_id");
					...
					this.product_object
						...
						.then((rows) => {
							if (!this._.isEmpty(rows)) {
								const product = rows;
								return this.render(res, "admin/edit-product", {
	...
	protected validatedEditProduct = () => ({
		...
		validate_product_id: check("product_id")
			.not()
			.isEmpty()
		...
		validate_description: check("description")
			.not()
			.isEmpty()
		...
		validate_image_path: check("uploaded_image")
			...
			.custom((value: any, { req }: Request) => {
				if (req.file) {
					if (
						req.file?.mimetype.includes(Singleton.getConstants().RESPONSE.TYPES.PNG) ||
					...
					if (typeof req.files["uploaded_image"] !== "undefined") {
						if (typeof req.files["uploaded_image"][Symbol.iterator] === "function") {
							req.files["uploaded_image"].forEach((image: any) => {
								if (
									!image.mimetype.includes(Singleton.getConstants().RESPONSE.TYPES.PNG) ||
	...
	postEditedProduct = () =>
		this.route(
			...
			async (req: Request, res: Response, next: NextFunction) => {
				...
				const title = this._.capitalize(req.getFormPostedData("title"));
				const price = req.getFormPostedData("price");
				const description = this._.capitalize(req.getFormPostedData("description"));
				...
				if (err instanceof multer.MulterError) {
					switch (err.code) {
						...
						case Singleton.getConstants().UPLOADER_ERRORS.FIELD_NAME_TOO_LONG:
							return this.onErrorValidation(
								res,
								"Field name is too long, please insert a short fields's name!"
							);
							break;
						case Singleton.getConstants().UPLOADER_ERRORS.FIELD_VALUE_TOO_LONG:
							return this.onErrorValidation(
								res,
								"Field value is too long, please insert a short fields's value!"
							);
							break;
						case Singleton.getConstants().UPLOADER_ERRORS.TOO_MANY_FIELDS:
							return this.onErrorValidation(
								res,
								"Alot of fields have been detected, please use less fields!"
							);
							break;
	...
	protected validatedDeleteProduct = () => ({
		isUserAuthenticated: isAuth,
		user_session: userSession,
		validate_title: check("product_id")
			.not()
			.isEmpty()
			.isNumeric()
	...
	products = () =>
		this.route(
			"get",
			"/admin/products/",
			{ cors: this.express.expressCors(this.corsOptionsDelegate), isAuth, userSession },
			async (req: Request, res: Response, next: NextFunction) => {
				if (!req.isGet()) {
					return this.siteNotFound(res);
				}
				...
				user_products
					.then((rows: any) => {
						return this.render(res, "admin/products", {
							products: rows || [],
