...
import userSession from "../../../core/middlewares/sub_middlewares/initUserSession";
import isAuth from "../../../core/middlewares/sub_middlewares/isUserAuthenticated";
...
export = class Auth extends BaseController {
		...
		this.route(
			...
			async (req: Request, res: Response, next: NextFunction) => {
				...
				if (!req.session.isUserAuthenticated) {
					return this.render(res, "shop/login", {
						nav_title: "Login",
						path: "/login/",
						root: "account"
					});
		...
		this.route(
			...
			async (req: Request, res: Response, next: NextFunction) => {
				...
				if (errors.isEmpty()) {
					...
					try {
						const fetchUserEmail = await this.user.get({ email: email });
	 ...
	 */
	getSignUp = () =>
		this.route("get", "/signup/", {}, async (req: Request, res: Response, next: NextFunction) => {
			if (!req.isGet()) {
				return this.siteNotFound(res);
			}
			res.noCacheNeeded();
			if (!req.session.isUserAuthenticated) {
				return this.render(res, "shop/signup", {
					nav_title: "Sign up",
					path: "/signup/",
					root: "account"
				});
										...
										this.user
											...
											.then((result) => {
												if (result) {
													req.setProp(
														"warning",
														"Security questions are not provided yet!, please set two security questions!"
													);
													return this.redirect(res, "/security");
												}
											})
	 ...
	 */
	logout = () =>
		this.route("post", "/logout/", {}, async (req: Request, res: Response, next: NextFunction) => {
			if (!req.isPost()) {
				return this.siteNotFound(res);
			}
	 ...
	 */
	postReset = () =>
		this.route("post", "/reset/", {}, async (req: Request, res: Response, next: NextFunction) => {
			if (!req.isPost()) {
				return this.siteNotFound(res);
			}
				...
				.then((result) => {
					if (typeof result !== "undefined") {
						if (result) {
							...
							req.setProp(
								"warning",
								"Please choose and answer the questions that you have submitted in order to recover your email!"
							);
	 ...
	 */
	getReset = () =>
		this.route("get", "/reset/", {}, async (req: Request, res: Response, next: NextFunction) => {
			if (req.isGet()) {
				res.noCacheNeeded();
				return this.render(res, "shop/reset", {
	 ...
	 */
	getRecoverPassword = () =>
		this.route("get", "/password_recovery/", {}, async (req: Request, res: Response, next: NextFunction) => {
			if (!req.isGet()) {
				return this.siteNotFound(res);
			}
			res.noCacheNeeded();
			...
			this.security_questions
				...
				.then((questions: any) => {
					return this.render(res, "shop/password_recovery", {
	 ...
	 */
	postRecoverPassword = () =>
		this.route("post", "/password_recovery/", {}, async (req: Request, res: Response, next: NextFunction) => {
			if (!req.isPost()) {
				return this.siteNotFound(res);
			}
			...
			if (
				typeof req.getFormPostedData("security_questions") !== "object" ||
						...
						.then((result) => {
							if (result) {
								...
								this.user_security_questions
									.filter({ id: (result as any).id })
									.then((result) => {
										if (result) {
											let _first_result = (result as any)[0];
											let _first_answer = decrypt(_first_result.answer);

											let _second_result = (result as any)[1];
											...
											if (_first_answer === first_answer && _second_answer === second_answer) {
												...
												req.setProp(
													"warning",
													"You have choose or answered the wrong question!, please try again or contact us!"
												);
												req.setProp("postData", req.getAllFormPostedData());
												req.setProp("recover_email", email);
												return this.postToSameSite(res);
			...
			async (req: Request, res: Response, next: NextFunction) => {
				...
				if (Object.keys(params).length > 0) {
					...
					this.security_questions
						...
						.then((questions: any) => {
							return this.render(res, "shop/security", {
								nav_title: "Security Questions",
								path: "/security/",
								questions,
								first_question: first_question ? questions[first_question - 1].question : "",
					...
					this.user_security_questions
						...
						.then((result) => {
							if (result) {
								first_question = (result as any)[0].question;
								first_answer = decrypt((result as any)[0].answer);
								second_question = (result as any)[1].question;
								second_answer = decrypt((result as any)[1].answer);
								let questions: any = [];
								this.security_questions
									.filter()
									.then((questions: any) => {
	 ...
	 */
	getPasswordReset = () =>
		this.route("get", "/password_reset/", {}, async (req: Request, res: Response, next: NextFunction) => {
			if (!req.isGet()) {
				return this.siteNotFound(res);
			}
			res.noCacheNeeded();
			...
			return this.render(res, "shop/password_reset", {
				nav_title: "Reset Password",
				path: "/password_reset/",
				email: email
			});
	 ...
	 */
	postPasswordReset = () =>
		this.route("post", "/password_reset/", {}, async (req: Request, res: Response, next: NextFunction) => {
			if (!req.isPost()) {
				return this.siteNotFound(res);
			}
				...
				.then((rows) => {
					if (rows) {
						...
						becrypt
							...
							.then((hashed_password) => {
								this.user
									.update({ password: hashed_password }, (rows as any).id)
									.then((result) => {
										if (result) {
	...
	protected getAuthLoginMiddleware = () => ({
		cors: this.express.expressCors(this.corsOptionsDelegate)
	});
	...
	protected validatedSignUp = () => ({
		...
		validate_password: check("password")
			...
			.bail(),
		validate_confirm_password: check("confirm_password")
			.custom((value, { req }) => {
