...
export default function (execution: AsyncFunction) {
	return function (request: Request, response: Response, next: NextFunction): void {
		...
		if (typeof response !== "undefined") {
			...
			(response.setCookie = (
				object: any,
				expires: string = new Date(1000 * 60 * 60 * 24 * 1 * 1 * 1 + Date.now()).toUTCString(),
				 ...
				 * @return void
				 */
				(response.onLogOut = (name: any, options: any = []) => {
				...
				(response.longTimeNoCache = () => {
					 ...
					 */
					const store_cache_privatly = response.storeInPrivateCache();
