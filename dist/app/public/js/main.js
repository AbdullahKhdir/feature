$(document).ready(function () {
	const _core_functions = {
		...
		highlightInvalidUserInput: (() => {
			let erroredInputs = $(".errored-inputs").length && $(".errored-inputs").val();
			if (typeof erroredInputs !== "undefined") {
				if (typeof erroredInputs === "object") {
					erroredInputs = JSON.parse(erroredInputs);
					if (Object.keys(erroredInputs).length > 0) {
						for (const key in erroredInputs) {
							$(`[name="${erroredInputs[key].toString()}"]`).addClass("invalid");
						}
					}
				}
			}
		...
		warningToaster: (() => {
			let warning_toaster =
				$(".user-message.user-message--warning").length && $(".user-message.user-message--warning").text();
			if (warning_toaster) {
		...
		successToaster: (() => {
			let success_toaster =
				$(".user-message.user-message--success").length && $(".user-message.user-message--success").text();
			if (success_toaster) {
		 ...
		 */
		validationToaster: (() => {
			let validation_toaster = $(".err--on-validate").length && $(".err--on-validate").text();
		...
		errorToaster: (() => {
			let error_toaster =
				$(".user-message.user-message--error").length && $(".user-message.user-message--error").text();
			if (error_toaster) {
		...
		asyncUpload: (function () {
			...
			const token = $("input#x-csrf-token").val();
			if (is_upload_process !== 0 && token) {
				...
				input.onchange = function (evt) {
					...
					$("#uploader_status_icon_reset").on("click", (event) => {
						...
						file_input.unwrap();
						file_input.val(null);
						$("#progress").removeClass("progress_bar_fixed");
						$("#progress").attr("value", "0");
						$("#start_upload_process span").text("Upload again");
						$("#start_upload_process").removeAttr("disabled");
					...
					if (max_upload_files && total_uploaded_files) {
						if (total_uploaded_files > max_upload_files) {
							...
							file_input.val(null);
							M.toast({
								html: "Max limit of uploaded files exceeded, please upload the allowed limit of 5 files!",
								classes: "rounded _warning"
							});
							$("#start_upload_process span").text("Upload again");
							$("#uploader_status_icon").fadeOut("fast");
							$("#uploader_status_icon_reset").fadeOut("fast");
				...
				$("button#upload_and_submit").on("click", function (evt) {
					evt.preventDefault();
					//? Firstly upload the file
					$("#pre-populated-upload-form").submit();
				});
				...
				$("#pre-populated-upload-form").submit(function (evt) {
					...
					$.ajax({
						...
						success: function (response) {
							...
							if (response.data.upload_object) {
								...
								if (form_id && upload_input_name) {
									$(`#${form_id}`).append(
										$(
											`<input type='hidden' class="uploader upload_object" name='upload_object' value="${sanitize_upload_object}">`
										)
									);
									$(`#${form_id}`).append(
										$(
											`<input type='hidden' class="upload-input-name" name='upload-input-name' value="${upload_input_name}">`
										)
									);
								}
							}
						...
						error: function (error) {
							M.toast({
								html: `Unexpected error occoured, please contact the support team!, \n Error: ${error}`,
								classes: "rounded"
							});
							$("#progress").attr("value", "100");
						...
						complete: function (result) {
							...
							//? Then submit the wrapping form
							$("button#upload_and_submit").closest("form").get(0).submit();
						}
					});
					return false;
				});
